	...
	<div class="masthead">
		...
		<div id="toolbar" class="row-fluid">
			<div class="span12 well">
				<form id="searchForm" class="form-inline" method="POST" action="/xml/processCalls/<?= $idmenu; ?>">
					<label>Range Date:</label>
					<div id="datetimepicker1" data-date-format="yyyy-mm-dd" data-date="<?= !empty($data['date1']) ? $data['date1'] : (!empty($userData['date1']) ? $userData['date1'] : date('Y-m-d')); ?>" class="input-append date">
					...
					<div id="datetimepicker2" class="input-append date" data-date="<?= !empty($data['date2']) ? $data['date2'] : (!empty($userData['date2']) ? $userData['date2'] : date('Y-m-d'));  ?>" data-date-format="yyyy-mm-dd">
				...
				<div class="pull-right	">
					<form id="formExport" class="form-inline" method="POST" action="/xml/menu2pdf/<?= $idmenu; ?>">
						...
						<input type="hidden" name="typeExport" value="call">
...
$(function() {
	...
	<?php 
			if(isset($_SESSION['call']['threshold'])){
		?>
			var setThreshold = new Array (
				<?= $_SESSION['call']['threshold'][0]; ?>,
				<?= $_SESSION['call']['threshold'][1]; ?>		
					);
	...
	$.ajax({
		...
		success: function(data){
			...
			?>
			$('.selectpicker').selectpicker('val',<?= $tmp; ?>);
			<?php 
			}
			?>
